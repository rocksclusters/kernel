...
static char **headers() {
    ...
    if (FL_KICKSTART_SEND_MAC(flags)) {
        ...
        for (i = 0; devices && devices[i]; i++) {
            ...
            logMessage(INFO, "ROCKS:headers: mac (%s)", mac);
            logMessage(INFO, "ROCKS:headers: drivername (%s)", drivername);
#endif
            if (mac) {
...
#ifdef	LATER
			devMakeInode(devices[i]->device, "/tmp/rocks-cdrom");
#endif
...
int getFileFromUrl(char * url, char * dest, 
    ...
    if (kickstartNetworkUp(loaderData, &iface)) {
        logMessage(ERROR, "ROCKS:getFileFromUrl:unable to bring up network");
        return(1);
    }
    ...
    if ((client = nm_client_new()) == NULL) {
        logMessage(ERROR, "ROCKS:getFileFromUrl:nm_client_new() failed");
        return(1);
    }

    if ((state = nm_client_get_state(client)) != NM_STATE_CONNECTED) {
        logMessage(ERROR, "ROCKS:getFileFromUrl:nm_client_get_state() failed");
...
dump_dhcp4_config (NMDHCP4Config *config)
{
	...
	g_print("ROCKS:dump_dhcp4_config:DHCP4 Options:\n");
