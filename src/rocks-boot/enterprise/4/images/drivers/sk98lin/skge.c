...
size_t		AllocLength;	/* length of complete descriptor area */
	...
	if (pAC->pDescrMem) {
#if (BITS_PER_LONG == 32)
		AllocLength = (RX_RING_SIZE + TX_RING_SIZE) * 
				pAC->GIni.GIMacsFound + 8;
#else
		AllocLength = (RX_RING_SIZE + TX_RING_SIZE) * 
				pAC->GIni.GIMacsFound + RX_RING_SIZE + 8;
#endif
...
DEV_NET       *pNet,  /* pointer to the DEV_NET structure of adapter */
unsigned int   Size,  /* length of IOCTL data in passed IOCTL struct */
