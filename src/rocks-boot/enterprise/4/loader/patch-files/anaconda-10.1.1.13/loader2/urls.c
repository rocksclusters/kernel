...
mountRocksDevice (char *device, int major, int minor, char *marker)
{
	...
	if (mknod(devicepath, mode, makedev(major, minor)) < 0) {
		logMessage("mountRocksDevice:mknod failed: %s",
			strerror(errno));
		return 1;
	}
	...
	if (!access(markerpath, F_OK)) {
		logMessage("mountRocksDevice: found rocks device "
			"at %s, mounted on %s", 
...
getPartitions()
{
	...
	if (fd<0) {
		logMessage("getPartitions:open failed "
			"for /proc/partitions");
		return 0;
	}
	...
	while (1) {
		...
		if (bytesread < 0) {
			logMessage("getPartitions:/proc read error");
			break;
		}
		if (!bytesread || (bytesread < chunk)) {
			...
			contents = (char*) realloc((void*) contents,
				contents_size);
			if (!contents) {
				logMessage("getPartitions:realloc error");
				return 0;
			}
			offset += bytesread;
...
getRocksUSB()
{
	...
	while (1) {
		...
		logMessage("ROCKS:searching for USB keys on device %s", dev);
