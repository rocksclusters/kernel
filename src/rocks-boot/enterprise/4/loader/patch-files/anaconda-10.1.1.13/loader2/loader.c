 ...
 */
#ifdef SDSC
#define ROCKS_SERVER "central-41.rocksclusters.org"
#define ROCKS_EXTERNAL_SUFFIX "install/sbin/wan/kickstart.cgi"
#endif
...
static int parseCmdLineFlags(int flags, struct loaderData_s * loaderData,
...
#ifdef SDSC
	else if (!strcasecmp(argv[i], "central")) {
		/* Get wide area ks info from a local dhcp svr. */
		loaderData->ksFile = "ks";
		loaderData->fromCentral = 1;
		logMessage("ROCKS:central:0");
	}
	else if (!strcasecmp(argv[i], "central=rocks")) {
		char	*p;

		p = (char*) malloc(strlen(ROCKS_SERVER)
			+ strlen(ROCKS_EXTERNAL_SUFFIX) + 12);

		sprintf(p, "ks=http://%s/%s",
			ROCKS_SERVER, ROCKS_EXTERNAL_SUFFIX);
		loaderData->ksFile = p;
		loaderData->fromCentral = 1;
		logMessage("ROCKS:central:1 server is %s", loaderData->ksFile);
	}
	else if (!strncasecmp(argv[i], "central=", 8)) {
		char	*p;

		p = (char*) malloc(strlen(argv[i])
			+ strlen(ROCKS_EXTERNAL_SUFFIX) + 12);

		sprintf(p, "ks=http://%s/%s",
			argv[i]+8, ROCKS_EXTERNAL_SUFFIX);
		loaderData->ksFile = p;
		loaderData->fromCentral = 1;
		logMessage("ROCKS:central:2 server is %s", loaderData->ksFile);
	}
	...
	else if (!strncasecmp(argv[i], "restore", 7)) {
		loaderData->restore = 1;
	}
#endif
...
#ifdef	SDSC
logMessage("ROCKS:anaconda exited");
logMessage("ROCKS:WIFEXITED(status) %d", WIFEXITED(status));
logMessage("ROCKS:WEXITSTATUS(status) %d", WEXITSTATUS(status));
logMessage("ROCKS:WIFSIGNALED(status) %d", WIFSIGNALED(status));
logMessage("ROCKS:WTERMSIG(status) %d", WTERMSIG(status));
#endif
