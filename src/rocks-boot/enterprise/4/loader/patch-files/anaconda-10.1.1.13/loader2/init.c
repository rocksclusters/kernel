...
static int dontReboot(void) {
	...
	if ((fd = open("/proc/cmdline", O_RDONLY,0)) > 0) {
		...
		if (len > 0 && mystrstr(buf, "rocks-debug")) {
			return 1;
		}
	}
	return 0;
...
#ifdef  SDSC
printf("ROCKS:childpid = %d\n", childpid);
printf("ROCKS:WIFEXITED(status) %d\n", WIFEXITED(waitStatus));
printf("ROCKS:WEXITSTATUS(status) %d\n", WEXITSTATUS(waitStatus));
printf("ROCKS:WIFSIGNALED(status) %d\n", WIFSIGNALED(waitStatus));
printf("ROCKS:WTERMSIG(status) %d\n", WTERMSIG(waitStatus));
#endif
