...
#ifdef	SDSC
{
	...
	*/
	...
	/* sacerdoti: If we are installing fresh from central, we know that a
	 * human will have to interact, so turn off the watchdog. 
	 */
	if (!haveCertificate() && loaderData->fromCentral 
				&& !loaderData->restore)
		watchdog_off();

	/* turn off parent authentication for LAN until we can 
	 * save the frontend's CA to cd or usb key. */
	if (loaderData->fromCentral && !loaderData->restore)
	...
	string_size = strlen(base) +
		strlen("?arch=&np=XXXX&project=&dist=&restore=yes") +
	...
	if (loaderData->restore)
		sprintf(file, "%s&restore=yes", file);
}
