...
int urlinstStartTransfer(struct iurlinfo * ui, char * filename, 
...
#ifdef	SDSC
	{
		...
		logMessage("ROCKS:urlinstStartTransfer:http://%s/%s\n"
			"Headers:%s\n",
		...
		while ((fd < 0) && (tries < 10)) {
			...
			logMessage("ROCKS:urlinstStartTransfer:attempt (%d)",
				tries);
		...
		if (fd < 0) {
			logMessage("ROCKS:urlinstStartTransfer:Failed");
...
bootable (char *device, int major, int minor)
{
	...
	if (mknod(devicepath, mode, makedev(major, minor)) < 0) {
		logMessage("bootable:mknod failed: %s",
			strerror(errno));
		return -1;
	}
	...
	if ((fd = open(devicepath, O_RDWR)) < 0) {
		logMessage("bootable:open %s failed: %s", 
	...
	if (read(fd, buf, sizeof(buf)) < 0) {
		logMessage("bootable:read failed: %s", strerror(errno));
		return -1;
	}
	...
	if (write(fd, buf, sizeof(buf)) < 0) {
		logMessage("bootable:write failed: %s", strerror(errno));
		return -1;
	}
	...
	if (ioctl(fd, BLKRRPART) != 0) {
		logMessage("bootable:ioctl failed: %s",
			strerror(errno));
		return -1;
	}
...
static int
isRocksDisk(char *mountpoint)
{
	char marker[128];

	snprintf(marker, 128, "%s/.rocks-release", mountpoint);
...
mountRocksDisk (char *device, int major, int minor)
{
	...
	if (mknod(devicepath, mode, makedev(major, minor)) < 0) {
		logMessage("mountRocksDisk:mknod failed: %s",
			strerror(errno));
		return 1;
	}

	mountpoint = "/mnt/rocks-old-disk";
	...
	for (i=0; fstype[i]; i++) {
		rc=doPwMount(devicepath, mountpoint, fstype[i],
			1, 0, NULL, NULL, 0, 0);
		if (!rc)
			break;
	}
	...
	if (isRocksDisk(mountpoint)) {
		logMessage("mountRocksDisk: found old root partition "
			"at %s, mounted on %s", 
 ...
 * Returns 1 on sucess, 0 otherwise. Mounts to a well-known dir.
 */
static int
getRocksPartition()
{
	...
	if (fd<0) {
		logMessage("mountRocksPartition:open failed "
			"for /proc/partitions");
		return 1;
	}
	...
	while (1) {
		...
		if (bytesread < 0) {
			logMessage("mountRocksPartition:/proc read error");
			break;
		}
		if (!bytesread || (bytesread < chunk)) {
			...
			contents = (char*) realloc((void*) contents,
				contents_size);
			if (!contents) {
				logMessage("mountRocksPartition:realloc error");
				return 1;
			}
			offset += bytesread;
	...
	while (1) {
		...
		if (!diskdevice || 
			...
			diskdevice = dev;
			logMessage("ROCKS:found disk device %s", dev);
			...
			logMessage("ROCKS:found partition %s", dev);
...
getCert(struct loaderData_s *loaderData)
{
	...
	if (getRocksPartition()) {
		if (ignoreCert()) 
			logMessage("ROCKS:getCert: ignoring old certificate");
		else
			logMessage("ROCKS:getCert: No Rocks disks found");
		return 1;
	}

	priv = "/mnt/rocks-old-disk/etc/security/cluster-cert.key";
	cert = "/mnt/rocks-old-disk/etc/security/cluster-cert.crt";
	ca = "/mnt/rocks-old-disk/etc/security/cluster-ca.crt";

	if (access(priv, R_OK) || access(cert, R_OK) || access(ca, R_OK)) {
		logMessage("ROCKS:getCert: could not read %s, %s, and %s",
			priv, cert, ca);
		umount("/mnt/rocks-old-disk");
		return -1;
	}
	loaderData->cert_filename = cert;
	loaderData->priv_filename = priv;
	loaderData->ca_filename = ca;
	logMessage("ROCKS:getCert: Found Keys on disk");
...
/* Returns true if we have a person checking the Certificate
...
checkingCert()
{
	...
	if (loaderData->fromCentral && !loaderData->cert_filename)
		return 1;
	else
		return 0;
}
...
addPublic(char **urlptr)
{
	...
	p = strrchr(url, '/');
	if (!p) {
		logMessage("addPublic:url not well formed.\n");
		return -1;
	}
...
urlinstStartSSLTransfer(struct iurlinfo * ui, char * filename, 
	...
	logMessage("ROCKS:transferring https://%s/%s\nHeaders:%s\n",
	       ui->address, filename, extraHeaders);
	...
	while ((errorcode < 0) && (tries < 10)) {
		...
		if (errorcode == FTPERR_FAILED_DATA_CONNECT) {
			 ...
			 * sleepmin and 60s. */
			sleeptime = sleepmin + 
				((60 - sleepmin) * (rand()/(float) RAND_MAX));
			sleep(sleeptime);
			tries = 1;
			continue;
		}
			...
			if (!haveCertificate())
				return NULL;	/* Rejected cert manually */

			rc = newtWinTernary(_("Skeptical"),
				_("Retry"), _("Proceed"), _("Cancel"),
				_("The security credentials of server '%s' do "
				"not match those saved on our last install.\n\n"
				"This can happen if you are installing from a "
				"different parent server, or if the desired "
				"server has been compromised."), ui->address);
			if (rc==1)	/* Retry */
				continue;
			else if (rc==2) {	/* Proceed */
				forceParentAuth();
				continue;
			}
			else if (rc==3)	/* Cancel */
		...
		/* Dont try multiple times if we are interactive */
		if (checkingCert())
			break;

		logMessage("ROCKS:urlinstStartSSLTransfer:attempt (%d)",
			tries);
	...
	if (errorcode < 0) {
		logMessage("ROCKS:urlinstStartSSLTransfer:Failed");
		rc = 0;
