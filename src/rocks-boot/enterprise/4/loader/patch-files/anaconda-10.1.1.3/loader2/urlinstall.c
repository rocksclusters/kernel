...
start_httpd()
{
	...
	for (i = 0; devs[i]; i++) {
		...
		devMakeInode(devs[i]->device, "/tmp/cdrom");

		logMessage("start_httpd:trying to mount device %s",
			devs[i]->device);
		if (doPwMount("/tmp/cdrom", "/mnt/cdrom", "iso9660", 1, 0,
				NULL, NULL, 0, 0)) {

			logMessage("start_httpd:doPwMount failed\n");
		} else {
			/*
			 * if there are multiple CD drives, exit this loop
...
#ifdef	SDSC
{
	...
	arch = "ia64";
	/*
	* XXX this is hardcoded for now. need to write a function
	* that finds the number of CPUs on an itanium
	*/
	np = 2;
	...
	/* If we are installing from central, we know that a human will
	 * have to interact, so turn off the watchdog. 
	 */
	if (!haveCertificate() && loaderData->fromCentral)
		watchdog_off();

	/* Sacerdoti: turn off parent authentication for LAN until we can 
	 * save the frontend's CA to cd or usb key. */
	if (loaderData->fromCentral)
		loaderData->authParent = 1;
